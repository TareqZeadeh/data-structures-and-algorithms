...
let findMaleAndFemale = (data) => {
  ...
  // return names;
  //===================================sol 2==============================================
  let filterednamse = data.filter(obj => (obj.gender === 'male' || obj.gender === 'female'));
  let names = filterednamse.reduce((acc, item) => {
    acc.push(item.name);
    return acc;
  }, []);
...
let findShortest = (data) => {
  ...
  // return name;
  //===============================sol 2=====================================
  let shortestHeight = Math.min(...data.map(obj => obj.height));
  let name = data.reduce((acc, cur) => {
    if (parseInt(cur.height) === shortestHeight) {
      acc = cur.name;
    }
    return acc;
  });
