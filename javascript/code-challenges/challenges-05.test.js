...
const toLastNames = people => {
  let newArr=[];
  people.map((arr,i)=>{
    newArr[i]=`${arr.firstName} ${arr.lastName}`;
  });
  return newArr;
};
...
const addPurchases = (arr) => {
  let sumPurch=arr.reduce((accumulator,value)=>{
...
const countNumberOfElements = (arr) => {
  let contentnum=arr.reduce((accumulator,value,i)=>{
    accumulator=i+1;
    return accumulator;
  },0);
  return contentnum;
...
const returnNames = (arr) => {
  let starWarsName=arr.reduce((accumulator,value,i)=>{
...
const reversedString = (str) => {
  ...
  let reversestr = arrstring.reduce((accumulator, value) => {
