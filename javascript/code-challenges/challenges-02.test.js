...
const raisedToTheThird = (arr) => {
  let raisarr = [];
  arr.forEach((number) => {
    raisarr.push(Math.pow(number, 3));
  });
  return raisarr;
...
const addOne = (arr) => {
  let incarr=arr.map((number) => number=number + 1);
  return incarr;
};
...
const addQuestion = (arr) => {
  let Qarr=arr.map((string) => string=string + '?');
  return Qarr;
};
...
const forLoopTwoToThe = (arr) => {
  ...
  for (let i = 0; i < arr.length; i++) {
    twotoarr.push(Math.pow(2, arr[i]));
  }
  return twotoarr;
};
/* ------------------------------------------------------------------------------------------------
...
const forEachTwoToThe = (arr) => {
  let twotoarr=[];
  arr.forEach((number)=>{
    twotoarr.push(Math.pow(2, number));
  });
  return twotoarr;
};
