...
const longestString = (arr) => {
  ...
  arr.forEach((element, idx) => {
    if (element.length > testlength) {
      testlength = element.length;
      index = idx;
    }
  });
  return index;
...
const findAnything = (arr, target) => {
  arr.filter(value => value.includes(target));
};
...
const findEvery = (arr, target) => {
  !arr.map(value => value.includes(target)).includes(false);
};
...
const unenrollBrook = (arr) => {
  arr.map(innerArr => innerArr.filter(value => !value.includes('Brook')));
};
